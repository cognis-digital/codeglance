...
How **codeglance** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `codeglance` lives in the **Agents + cognition** cluster.
...
| from | relation | to |
|---|---|---|
| `codeglance` | JSON findings compose with | [`agentlex`](https://github.com/cognis-digital/agentlex), [`humind`](https://github.com/cognis-digital/humind), [`agentsmith`](https://github.com/cognis-digital/agentsmith), [`agentmap`](https://github.com/cognis-digital/agentmap), [`engram`](https://github.com/cognis-digital/engram) |
| `codeglance` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `codeglance` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `codeglance` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the agents + cognition cluster.** `codeglance` output feeds the next tool:
```bash
codeglance ... --format json > out.json          # this tool's findings (see `codeglance --help`)
...
codeglance ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
codeglance ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
codeglance ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `codeglance`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `codeglance` slots
