...
**codeglance** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `codeglance` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
codeglance ... --format json | cognis-connect emit --to brief   # analyst brief via your /v1 fleet
codeglance ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `codeglance` outputs:

```python
from cognis_connect import normalize, edgemesh
findings = [normalize(rec, source="codeglance") for rec in records]   # records = your JSON output
