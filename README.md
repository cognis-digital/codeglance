...
<img src="https://capsule-render.vercel.app/api?type=rect&color=0:6b46c1,100:2b6cb0&height=120&section=header&text=CODEGLANCE&fontSize=48&fontColor=ffffff&fontAlignY=58" width="100%" alt="CODEGLANCE"/>
...
AI-agent onboarding niche

`codeglance` is single-purpose, scriptable, and self-hostable: point it at a target, get prioritized results in the format your workflow already speaks (table · JSON · SARIF), gate CI on it, and let agents drive it over MCP.
...
**Topics:** `cognis` `devtools` `cloud` `developer-tools` `agent-security`
...
codeglance scan . --fail-on high        # CI gate (non-zero exit)
```
...
`codeglance` is interoperable with every popular way of using AI:

- **MCP server** — `codeglance mcp` (Claude Desktop, Cursor, Cognis.Studio, [uncensored-fleet](https://github.com/cognis-digital/uncensored-fleet))
- **OpenAI-compatible / JSON** — pipe `codeglance scan . --format json` into any agent or LLM
...
*Built in the spirit of **code2flow**, re-framed the Cognis way. Missing a credit? Open a PR.*
...
Pipes into your stack: **SARIF** for code-scanning, **JSON** for anything, an **MCP server** (`codeglance mcp`) for AI agents, and a webhook forwarder for SIEM/Slack/Jira. See [`docs/INTEGRATIONS.md`](docs/INTEGRATIONS.md).
...
```bash
pip install "git+https://github.com/cognis-digital/codeglance.git"    # pip (works today)
pipx install "git+https://github.com/cognis-digital/codeglance.git"   # isolated CLI
uv tool install "git+https://github.com/cognis-digital/codeglance.git" # uv
pip install cognis-codeglance                                          # PyPI (when published)
docker run --rm ghcr.io/cognis-digital/codeglance:latest --help        # Docker
brew install cognis-digital/tap/codeglance                             # Homebrew tap
curl -fsSL https://raw.githubusercontent.com/cognis-digital/codeglance/main/install.sh | sh
```

| Linux | macOS | Windows | Docker | Cloud |
|---|---|---|---|---|
| `scripts/setup-linux.sh` | `scripts/setup-macos.sh` | `scripts/setup-windows.ps1` | `docker run ghcr.io/cognis-digital/codeglance` | [DEPLOY.md](docs/DEPLOY.md) (AWS/Azure/GCP/k8s) |
...
> ### ⭐ If `codeglance` saved you time, **star it** — it genuinely helps others find it.
