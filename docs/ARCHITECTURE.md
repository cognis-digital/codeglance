...
- **rules/analyzers** apply the heuristics shipped in `codeglance/core.py`.
- **score** ranks by severity.
- **MCP server** (`codeglance mcp`) exposes `scan` for Cognis.Studio agents.
